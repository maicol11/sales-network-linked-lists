...
public class Companie {
	...
	void totalBalance(Seller seller) {
		...
		Seller tmpright = seller.right;
		float balanceRight = 0;
		while(tmpright != null) {
			balanceRight += sellerBalance2(tmpright);
			tmpright = tmpright.right;	
		}
		Seller tmpleft = seller.left;
		float balanceLeft = 0;
		while(tmpleft != null) {
			balanceLeft += sellerBalance2(tmpleft);
			tmpleft = tmpleft.left;
		}
		float total = balance + balanceRight + balanceLeft;
	...
	float biggerBalance(Seller seller) {
		...
		Seller tmpleft = seller.left;
		while(tmpleft != null) {
			float balanceLeft = sellerBalance2(tmpleft);
			if(balanceLeft > tmp ) {
				tmp = balanceLeft;
			}
			tmpleft = tmpleft.left;	
